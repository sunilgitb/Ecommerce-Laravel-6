...
class ProductController extends Controller
{
    public function ProductView($id,$product_name){
        ...
        // $rating_sum = Rating::where('product_id', $product_id)->get();
        // $rating_value = $rating_sum / $ratings->count();
        // $user_rating = Rating::where('product_id', $product_id)->where('user_id', Auth::id())->first();
        // if($ratings->count() > 0)
        // {
        //     $rating_value = $rating_sum/$ratings->count();
        // }
    	...
    	return view('pages.product_details',compact('product','product_color','product_size'))->render();
...
$product = DB::table('products')->where('id',$id)->first();
 ...
 if ($product->discount_price == NULL) {
  ...
  public function CategoryView($id){
    ...
    return view('pages.all_category2',compact('category_all'));
